    ...
    $fieldset_id = 5  # Change this to the appropriate fieldset ID for your models (Custom Fields)
    ...
    <# function Get-MacAddresses {
        $macAddresses = Get-WmiObject -Class Win32_NetworkAdapterConfiguration | Where-Object { $_.MACAddress -ne $null -and $_.Description -like "*PanGP*"} | Select-Object -ExpandProperty MACAddress
        return $macAddresses -join ", "
    } #>

    function Get-VPNMacAddress {
        $vpn_macAddress = (reg query "HKLM\Software\Palo Alto Networks\GlobalProtect\PanGPS" /v mac-addr 2>$null) -match "mac-addr\s+REG_BINARY\s+([0-9A-Fa-f]+)" | Out-Null; $matches
        return $vpn_macAddress

    }
    function Get-ActiveIPAddress {
        $ipAddress = (Get-WmiObject -Class Win32_NetworkAdapterConfiguration | Where-Object { $_.IPEnabled -eq $true -and $_.Description -like "Juniper Networks Virtual*" } | Select-Object -ExpandProperty IPAddress)
        return $ipAddress
    }
    ...
    function Get-VPNIPAddress {
        $vpn_ipAddress = (Get-WmiObject -Class Win32_NetworkAdapterConfiguration | Where-Object { $_.IPEnabled -eq $true -and $_.Description -like "PANGP*" } | Select-Object -ExpandProperty IPAddress)
        return $vpn_ipAddress
    }

    #Function to get the ICCID of the MDC
    function Get-ICCID{
    $MDC_ICCID = (netsh mbn show readyinfo interface="Cellular*") -match "SIM ICC ID" | ForEach-Object { ($_ -split ':')[1].Trim()}
    return $MDC_ICCID
    }

    function Get-IMEI{
        $imeiQuery = netsh mbn show interfaces interface="cellular"
        $MDC_IMEI = ($imeiQuery | Select-String "Device Id").ToString() -replace '.*:\s*', ''
        return $MDC_IMEI
        }

    function get-mdcphone{
        $MDC_MobilePhone1 = (netsh mbn show readyinfo interface="cellular") -match "Telephone #1" | ForEach-Object { ($_ -split ':')[1].Trim()}
        return $MDC_MobilePhone1
    ...
    function Get-CustomFields {
        try {
            ...
            $vpn_ipAddress = Get-VPNIPAddress
            $vpn_macAddress = Get-VPNMacAddress
            $MDC_ICCID = Get-ICCID
            $MDC_IMEI = Get-IMEI
            $MDC_Phone = get-mdcphone

            # Validate each custom dbfield names : https://snipe-it.readme.io/reference/hardware-create
            $dbFields = @{
               # "_snipeit_adresse_mac_13"   = if ($macAddresses) { $macAddresses } else { "" }
            # "_snipeit_ram_5"           = if ($ramAmount) { $ramAmount } else { "" }
                ...
                #"_snipeit_pan_gp_mac_address_15" = if ($macAddresses) { $macAddresses } else { "" }
                "_snipeit_vpn_ip_address_10" = if ($vpn_ipAddress) { $vpn_ipAddress } else { "" }
                "_snipeit_iccid_8" = if ($MDC_ICCID) { $MDC_ICCID } else { "" }
                "_snipeit_imei_7" = if ($MDC_IMEI) { $MDC_IMEI } else { "" }
                "_snipeit_palo_alto_macaddress_9" = if ($vpn_macAddress) { $vpn_macAddress } else { "" } 
                "_snipeit_phone_numbers_15" = if ($MDC_Phone) { $MDC_Phone } else { "" } 
            }
            return $dbFields
        } catch {
