    ...
    function Get-IMEI{
    $MDC_IMEI = (netsh mbn show readyinfo interface="Cellular") -match "Subscriber ID" | ForEach-Object { ($_ -split ':')[1].Trim()}
    return $MDC_IMEI
    }
        ...
        try {
           ...
           # $ipAddress = Get-ActiveIPAddress
            ...
            # "_snipeit_ram_5"           = if ($ramAmount) { $ramAmount } else { "" }
                ...
                "_snipeit_vpn_ip_address_16" = if ($vpn_ipAddress) { $vpn_ipAddress } else { "" }
                "_snipeit_iccid_8" = if ($MDC_ICCID) { $MDC_ICCID } else { "" }
                "_snipeit_imei_7" = if ($MDC_IMEI) { $MDC_IMEI } else { "" }
                "_snipeit_pan_gp_mac_address_15" = if ($vpn_macAddress) { $vpn_macAddress } else { "" } 
            }
            return $dbFields
        } catch {
